...
l4 = size(perms4);
l5 = size(perms5);
class_setup_log_regress;
usepat = double(usepat);
load prereq_data;
n = 1
while(n <= 100)
    testvec = vals;
    ...
    [B,dev,stats] = mnrfit(data,usepat);
    for j = 1:5
        [value, arg] = max(mnrval(B, testvec(j,:)));
        if(arg == j)
    ...
    for i = 1:l1(1)
        ...
        [B,dev,stats] = mnrfit(data2,usepat);
        for j = 1:5
            [value, arg] = max(mnrval(B, testvec2(j,:)));
            if(arg == j)
